...
export type AbilityForClass<T extends CharacterClass> = 
...
export type CharacterWithStats<T extends Character> = T & BaseStats & {
...
export type Item = 
  | { type: 'weapon'; name: string; damage: number; rarity: 'common' | 'rare' | 'epic' | 'legendary' }
  | { type: 'armor'; name: string; defense: number; durability: number }
  | { type: 'potion'; name: string; effect: 'heal' | 'mana' | 'strength'; amount: number }
