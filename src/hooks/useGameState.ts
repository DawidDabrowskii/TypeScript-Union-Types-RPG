...
import { Character, CharacterWithStats, GameState, StatusEffect, Element } from '../types/game';
import { createSpellName } from '../utils/typeGuards';

// Utility type for creating initial characters
type CharacterFactory<T extends Character['type']> = () => CharacterWithStats<Extract<Character, { type: T }>>;
...
export const useGameState = () => {
  const [gameState, setGameState] = useState<GameState>(() => ({
    ...
    inventory: [
      ...
      { type: 'scroll', name: 'Lightning Scroll', spell: createSpellName('lightning') }
    ],
    gamePhase: 'setup'
  }));
  ...
  const addRandomStatusEffect = useCallback(() => {
    ...
    setGameState(prev => {
      ...
      updatedCharacters[prev.currentCharacter] = {
        ...updatedCharacters[prev.currentCharacter],
        statusEffects: [...updatedCharacters[prev.currentCharacter].statusEffects, randomEffect]
      };
